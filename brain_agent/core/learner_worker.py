...
class LearnerWorker:
    ...
    def _prepare_train_buffer(self, rollouts, macro_batch_size):
        ...
        tensors_to_squeeze = self.actor_critic.get_tensors_to_squeeze()
    ...
    def _entropy_exploration_loss(self, action_distribution, exclude_last=False, mask=None):
    ...
    def _train(self, mb):
        ...
        log_prob_actions = self.actor_critic.get_log_prob_action(result, mb)
        ratio = torch.exp(log_prob_actions - mb.log_prob_actions)  # pi / pi_old

        # super large/small values can cause numerical problems and are probably noise anyway
        ratio = torch.clamp(ratio, 0.000001, 20.0)
        ...
        with self.timing.timeit('losses'):
            policy_loss = self._policy_loss(ratio, log_prob_actions, adv.detach(), clip_ratio_low, clip_ratio_high,
                                            ppo=self.cfg.learner.use_ppo, exclude_last=self.cfg.learner.exclude_last)
            exploration_loss = self.actor_critic.get_exploration_loss(self.exploration_loss_func, result, mb,
                                                                      exclude_last=self.cfg.learner.exclude_last)
            ...
            if self.cfg.model.use_popart:
                ...
                value_loss = self._value_loss(values, old_values, targets, clip_value, ppo=self.cfg.learner.use_ppo,
                                              exclude_last=self.cfg.learner.exclude_last)
        ...
        kl_old, kl_old_mean = self.actor_critic.get_kl_divergence(result, mb)
    ...
    def _record_summaries(self, train_loop_vars):
        ...
        stats = AttrDict()
        self.actor_critic.update_stats(var, stats)
    ...
    def load_from_checkpoint(self):
        ...
        if checkpoint_dict is None or not self.cfg.learner.resume:
    ...
    def initialize(self):
        ...
        elif self.cfg.optim.type == 'lamb':
            lr = 1e-7 if self.cfg.optim.scheduler == 'cosine' else self.cfg.optim.learning_rate
            self.optimizer = torch_optimizer.Lamb(
                params,
                lr=lr,
            )
        else:
        ...
        if self.cfg.optim.scheduler == 'cosine':
            from brain_agent.utils.optim_utils import CosineAnnealingWarmUpRestarts
            self.scheduler = CosineAnnealingWarmUpRestarts(
                self.optimizer,
                self.cfg.optim.train_for_env_steps,
                T_mult=1,
                eta_max=self.cfg.optim.learning_rate,
                T_up=int(self.cfg.learner.warmup_optimizer * self.cfg.optim.batch_size),
                gamma=1.,
                last_epoch=-1
            )
        elif self.cfg.optim.scheduler is None:
            self.scheduler = None
        else:
            raise NotImplementedError
