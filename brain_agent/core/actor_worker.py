...
from brain_agent.core.models.rnn import RNN_MODULES
...
class VectorEnvRunner:
    ...
    def init(self):
        ...
        for env_i in range(self.num_envs):
            ...
            if not 'nethack' in self.cfg.env.name:
                env.seed(env_id)
    ...
    def _process_policy_outputs(self):
        ...
        for env_i in range(self.num_envs):
            ...
            for tensor_idx, name in enumerate(actor_state.policy_output_names):
                if name == 'rnn_states' and self.cfg.model.core.core_type in RNN_MODULES:
            ...
            if self.cfg.model.agent == 'nethack_baseline_agent':
                actor_state.prev_actions = actor_state.last_actions
                actor_state.last_actions = policy_outputs_dict['actions']
            else:
                actor_state.last_actions = dict(
                    atype=policy_outputs_dict['atypes'],
                    direction=policy_outputs_dict['directions'],
                    spell=policy_outputs_dict['spells'],
                    use_item=policy_outputs_dict['use_items'],
                    pick_item=policy_outputs_dict['pick_items']
                )
                actor_state.prev_actions = actor_state.last_actions

            if self.cfg.model.core.core_type in RNN_MODULES:
                actor_state.last_rnn_state = new_rnn_state

    def _process_reward(self, rewards):
        rewards = rewards * self.cfg.env.reward_scale
        rewards = np.clip(rewards, -self.cfg.env.reward_clip, self.cfg.env.reward_clip)
    ...
    def _get_actions(self, env_idx):
        if self.cfg.model.agent == 'nethack_baseline_agent':
            actions = self.actor_states[env_idx].last_actions.type(torch.int32).numpy().item()
        elif self.cfg.model.agent == 'nethack_separated_action_agent':
            actions = {
                'atype': self.actor_states[env_idx].last_actions['atype'].type(torch.int32).numpy()[0],
                'direction': self.actor_states[env_idx].last_actions['direction'].type(torch.int32).numpy()[0],
                'spell': self.actor_states[env_idx].last_actions['spell'].type(torch.int32).numpy()[0],
                'use_item': self.actor_states[env_idx].last_actions['use_item'].type(torch.int32).numpy()[0],
                'pick_item': self.actor_states[env_idx].last_actions['pick_item'].type(torch.int32).numpy()[0],
            }
        else:
    ...
    def advance_rollouts(self, data, timing):
        ...
        for env_i, e in enumerate(self.envs):
            with timing.timeit('_env_step'):
                actions = self._get_actions(env_idx=env_i)
...
class ActorState:
    def __init__(self, cfg, env, actor_idx, split_idx, env_idx, traj_tensors, num_traj_buffers, policy_outputs_info,
        ...
        if not self.cfg.model.core.core_type in RNN_MODULES:
