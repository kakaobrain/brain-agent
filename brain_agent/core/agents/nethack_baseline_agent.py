...
class NetHackBaselineAgent(ActorCriticBase):
        ...
        if self.cfg.model.extended_input_action or self.cfg.model.extended_input:
            if self.cfg.model.use_prev_action_emb:
                self.prev_action_encoder = ActionBaseModel(
                    action_space.n, self.cfg.model.prev_action_emb_dim, self.cfg.model.prev_action_use_index_select)
        ...
        if self.cfg.model.extended_input or self.cfg.model.extended_input_action:
            ...
            if self.cfg.model.use_prev_action_emb:
                if self.cfg.model.extended_input:
                    x = torch.cat((x, self.prev_action_encoder(actions), rewards.clamp(-1, 1).unsqueeze(1)), dim=1)
                elif self.cfg.model.extended_input_action:
                    x = torch.cat((x, self.prev_action_encoder(actions)), dim=1)
            else:
                if self.cfg.model.extended_input:
    ...
    def forward_tail(self, core_output, task_ids, with_action_distribution=False):
        ...
        avail = self.obs_dict['avail_atype'] if 'avail_atype' in self.obs_dict else None
        action_distribution_params, action_distribution = self.action_parameterization(core_output, avail)
